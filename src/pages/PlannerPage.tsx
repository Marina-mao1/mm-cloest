...
import type { Outfit, PlannerDay, WeeklyPlan } from "../types";
...
export function PlannerPage({ outfits, weeklyPlan, onAssign }: Props) {
  const byId = new Map(outfits.map((item) => [item.id, item]));
  return (
    <div>
      <div className="soft-shell mb-5 p-6">
        <p className="mb-2 inline-flex rounded-full border border-ink bg-butter px-3 py-1 text-xs font-semibold text-ink shadow-sm">MM weekly board</p>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        {PLANNER_DAYS.map((day) => {
          ...
          const gym = day === "Monday" || day === "Wednesday";
          return (
            <section key={day} className="cute-panel p-4">
              <h2 className="font-medium">{day}</h2>
              <p className="text-sm text-[#8a7488]">{DAY_LABELS[day]}</p>
              {gym && <div className="mt-3 flex items-center gap-2 rounded-2xl bg-moss/10 px-3 py-2 text-sm text-moss"><Dumbbell size={16} />今天晚上健身</div>}
